...
class ApiServices {
  ...
  static Future<List<Mahasiswa>> getMahasiswa() async {
    try {
      // jia berhasil konek ke baseUrl
      final response = await http.get(Uri.parse(baseUrl));
      if (response.statusCode == 200) { // respon code 200 ini berhasil
  ...
  try {
    final response = await http.post(
      Uri.parse(baseUrl),
      headers: {'Content-Type': 'application/json'}, // untuk mengatakan ke server bahwa data yagn dikirim ad
